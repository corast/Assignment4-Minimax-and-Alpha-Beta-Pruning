...
class MinimaxAgent(MultiAgentSearchAgent):
	...
	"""
        def minimax(gameState, depth, max, agent=0):
            bestValue = None
            if max:
                agent = 0
            # Check if we are at goelStaet
            # print "gameState.isWin()" gameState.isWin()
            if (gameState.isWin or gameState.isLose or depth == 0):
                # Basecase for our recursion
                return self.evaluationFunction(gameState)
            # Need to check wheter we are at max or at min.
            if max:
                # best value =
                bestValue = sys.maxint
                #-Number.MAX_VALUE
                # Recurser alle barnene, alts alle mulige veier fra denne.

                for action in gameState.getLegalActions(agent):
    				value = minimax(gameState, depth - 1, False, agent + 1)
	...
	"""
		...
		def minPlayer(gameState, depth, ghost_index):
			...
			if (ghost_index == gameState.getNumAgents()-1):
    			for action in legalActions:
    				...
    				value = min(value, maxPlayer(gameState, depth-1))
			else:
    			for action in legalActions:
    				nextState = gameState.generateSuccessor(ghost_index, action)
        ...
        # print "eval", self.evaluationFunction()
        ...
        for action in legalActions:
			...
			score = max(score, MinimaxAgent(nextState,self.depth,1))
			if(score > prevscore):
    			bestAction = action
        return bestAction
